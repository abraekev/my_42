...
static void innit_size(Chunk *min, Chunk *mid, Chunk *max)
{
    min->size = 0;
    mid->size = 0;
    max->size = 0;
...
static void set_split_loc(Location loc, Chunk *min, Chunk *mid, Chunk *max)
{
    if (loc == TOP_A)
    {
        min->loc == BOTTOM_B;
...
static void set_third_pivots(
    Location loc, int chunk_size, int *pivot_1, int *pivot_2)
{
...
void chunk_split(Data *data, Chunk *chunk, Destination *dest)
{
    ...
    while (chunk->size--)
    {
        ...
        if (next_value > next_value - pivot_2)
        {
            ...
            dest->mid.size += move_from_to(data, chunk->loc, dest->mid.size);
        else
